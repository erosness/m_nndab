...
struct Frontier_format {
  char message_length[2];
  char identifier[2];
  char command;
  char node[5];
  char data[1024];
...
union Frontier_message {
  struct Frontier_format message;
  char msg_data[sizeof(struct Frontier_format)];
};
...
int main(int argc, char **argv) {
  ...
  int socket = nn_socket(AF_SP, NN_REP);
    ...
    union Frontier_message tx_data;
  ...
  tx_data.message.message_length[0] = 0;
  tx_data.message.message_length[1] = 8;
  tx_data.message.identifier[0] = 0x03;
  tx_data.message.identifier[1] = 0x01;
  tx_data.message.command = 0x00;
  tx_data.message.node[0] = 0x01;
  tx_data.message.node[1] = 0x03;
  tx_data.message.node[2] = 0x09;
  tx_data.message.node[3] = 0x04;
  tx_data.message.node[4] = 0x00;

  dab_flush(venice_fd);
  write(venice_fd,tx_data.msg_data,10);
  retries = 10000;
